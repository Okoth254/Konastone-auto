...
export default async function ReviewModerationDetail({ params }: { params: Promise<{ id: string }> }) {
    ...
    const customerName = review.customer_name || review.reviewer_name || 'Customer';
    const reviewText = review.review_text || review.content || review.comment || '';
    ...
    const vehicleName = review.vehicles
        ...
        : review.vehicle_make || review.vehicle_model
            ? `${review.vehicle_make || ''} ${review.vehicle_model || ''}`.trim()
            : 'General Review';
                ...
                <div className="flex items-center gap-4 lg:gap-10 w-full sm:w-auto">
                    ...
                    <div className="space-y-1">
                        <h2 className="text-xl font-heading font-black text-white uppercase tracking-tighter italic">
                            REVIEW: <span className="text-primary">{customerName}</span>
                        </h2>
                        <div className="flex flex-wrap items-center gap-3">
                            <span className="text-[9px] font-black font-mono text-slate-500 uppercase tracking-widest">REVIEW ID: {review.id.substring(0, 12).toUpperCase()}</span>
                            <span className="w-1 h-1 rounded-full bg-slate-800" />
                            <span className="text-[9px] font-black font-mono text-slate-500 uppercase tracking-widest">STATUS: {reviewStatus.toUpperCase()}</span>
                        </div>
                    </div>
                </div>
                <div className="hidden md:flex items-center gap-3">
                    <MotionBadge color={reviewStatus === 'approved' ? 'primary' : reviewStatus === 'pending' ? 'accent' : 'danger'}>
                        {reviewStatus}
                    </MotionBadge>
                </div>
            </motion.header>
                ...
                <div className="col-span-12 xl:col-span-4 space-y-10">
                    ...
                    >
                        ...
                        <div className="p-5 sm:p-6 lg:p-8 space-y-8">
                            <div>
                                <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em] mb-2">Vehicle</p>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-white/2 border border-white/5 rounded-2xl p-4">
                                    ...
                                    <span className={`text-xs font-black tracking-widest uppercase ${reviewStatus === 'pending' ? 'text-primary' : reviewStatus === 'approved' ? 'text-accent-teal' : 'text-red-500'}`}>{reviewStatus}</span>
                                </div>
                                <div className="bg-white/2 border border-white/5 rounded-2xl p-4">
                            ...
                            <div className="flex items-center gap-2 text-slate-500">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.3em]">Public review moderation</span>
                            </div>
                        </div>
                    </motion.section>
                </div>
                        ...
                        <div className="relative z-10 space-y-10">
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 lg:gap-8">
                                <div className="flex items-center gap-6">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em] mb-2">Customer</p>
                                        <h1 className="font-heading text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter text-white uppercase italic leading-none">{customerName}</h1>
                                ...
                                <div className="md:text-right text-left">
                                    <span className="block text-[10px] text-slate-500 uppercase tracking-widest font-black">Submission Date</span>
                            ...
                            <div className="flex flex-wrap gap-2">
                                <MotionBadge color="neutral">SERVICE</MotionBadge>
                                <MotionBadge color="neutral">VEHICLE QUALITY</MotionBadge>
                                <MotionBadge color="neutral">CUSTOMER EXPERIENCE</MotionBadge>
                            </div>
                        </div>
                    </motion.section>
