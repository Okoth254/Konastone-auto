                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em]">PUBLISHED: {availableCount}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-blue-500/50 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
                            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em]">IN TRANSIT: {inTransitCount}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4 bg-surface-dark/40 backdrop-blur-xl p-2 rounded-2xl border border-white/5 w-full md:w-auto">
                    <div className="relative group">
                        <button className="h-12 px-4 sm:px-6 flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-300 hover:text-white transition-all w-full sm:min-w-[200px]">
                            {currentSort === 'price_asc' ? 'PRICE: LOW-HIGH' : currentSort === 'price_desc' ? 'PRICE: HIGH-LOW' : 'SORT: NEWEST'}
                        ...
                        <div className="absolute top-full right-0 mt-2 w-full bg-surface-dark border border-white/10 rounded-2xl overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 shadow-2xl">
                            ...
                            <Link href={getSortLink('price_desc')} className="block px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 hover:bg-white/2 hover:text-primary border-b border-white/5 transition-all">Highest Price</Link>
                            <Link href={getSortLink('price_asc')} className="block px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 hover:bg-white/2 hover:text-primary transition-all">Lowest Price</Link>
                        </div>
                    </div>
                    ...
                    >
                        <div className="space-y-6">
                            ...
                            <div className="flex flex-col gap-2">
                                {[
                                    { label: 'ALL VEHICLES', value: undefined, icon: 'grid_view' },
                                    { label: 'AVAILABLE', value: 'available', icon: 'verified' },
                                    { label: 'IN TRANSIT', value: 'in_transit', icon: 'local_shipping' },
                                    { label: 'SOLD', value: 'sold', icon: 'history' }
                                ].map((filter) => (
                                    <Link
                                        key={filter.label}
                                    ...
                                    <div className="p-5 sm:p-6 lg:p-8 flex-1 flex flex-col gap-6 lg:gap-8">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                            <MotionButton
                                                ...
                                                variant="outline"
                                                className="h-12 border-white/10 hover:border-primary/40 group/btn"
                                            >
